...
A `pre-push` git hook (installed via the root `prepare` script, `.githooks/pre-push`) runs core unit tests and the Playwright E2E suite before every push; expect pushes to take longer than the raw git operation. Both steps are wrapped in `scripts/time-cmd.sh`, which appends a start/end/duration line to `test-timing/containers.log` (gitignored) — check that file if a push feels unusually slow.
