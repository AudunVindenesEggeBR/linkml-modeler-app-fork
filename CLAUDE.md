...
**A literal `{{` / `}}` anywhere in `packages/docs/*.md` prose — including inside single-backtick inline code, not just unescaped plain text — breaks the VitePress build.** VitePress compiles each Markdown page as a Vue SFC template, and Vue's compiler tries to parse anything matching `{{ ... }}` as a JS interpolation expression, wherever it occurs in the resulting HTML — inline `` `code` `` spans are not automatically exempted, only proper triple-backtick fenced code blocks reliably are. Confirmed empirically (2026-09-18): `packages/docs/development.md` had a Podman Go-template format string, `` `--format '{{.Image}}'` ``, written as ordinary inline code in a prose paragraph — this built and rendered fine locally in every other check (nothing else touches `packages/docs`) but broke `deploy-docs.yml` in CI with `[vite:vue] Error parsing JavaScript expression: Unexpected token`, since `.Image` isn't valid JS on its own. Fixed by replacing the inline-code span with raw HTML using `v-pre` (which tells Vue's compiler to skip that node) plus `&lt;`/`&gt;` entities for the angle brackets: `<code v-pre>podman inspect &lt;container&gt; --format '{{.Image}}'</code>` — verified by rebuilding and grepping the rendered HTML to confirm the literal command text survived. If you need to show literal `{{ }}` syntax (Go templates, Helm, Jinja2, Vue itself) in `packages/docs` prose, use this `<code v-pre>` pattern rather than a plain backtick span, and rebuild locally (`pnpm --filter @linkml-editor/docs build`) before assuming a docs change is safe — this class of break is invisible to `pnpm lint`/`pnpm test` and was previously only ever caught by CI, after merge.
